...
(define (row i) (vector-ref data i))

(define (col j)
  (vector-map (lambda (row) (vector-ref row j)) data)) 

(define (el i j) (vector-ref (row i) j))
    
(define (get-right i j)
  (let ((v (row i)))
    (if (> (+ j 4) (vector-length v))
      '#()
      (subvector v j (+ j 4)))))
...
(define (vector-prod v)
  (cond
    ...
    ((= (vector-length v) 1)
     (vector-first v))
    (else
     (* (vector-first v) (vector-prod (vector-tail v 1))))))
  ...
  (let loop
   ...
   (cond ((and (= i 19) (= j 20))
         ...
         (else
           (loop i (+ j 1) (max
                             maximum
                             (vector-prod (get-right i j))
                             (vector-prod (get-down i j))
                             (vector-prod (get-diagNE i j))
                             (vector-prod (get-diagSW i j))))))))
